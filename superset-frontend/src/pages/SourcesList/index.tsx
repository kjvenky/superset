...
import {
  ...
  function SourcesList(props: ChartListProps) {
    const {
      addDangerToast,
      addSuccessToast,
      user: { userId },
    ...
    function handleBulkChartDelete(chartsToDelete: Chart[]) {
      SupersetClient.delete({
        endpoint: `/api/v1/sources/?q=${rison.encode(
    ...
    if (canCreate) {
      subMenuButtons.push({
        name: (
          <>
            <i className="fa fa-plus" /> {t('Source')}
          </>
        ),
        buttonStyle: 'primary',
        onClick: () => {
    ...
    return (
      <>
        <SubMenu name={t('Sources')} buttons={subMenuButtons} />
        {sliceCurrentlyEditing && (
          <PropertiesModal
  ...
  export default withToasts(SourcesList);
